        ...
        if (target != current) {
            ...
            if (w != null) {
                ...
                if (isBreakable(w, s, block) && w.destroyBlock(block, false, entity)) {
                    Multiverse.CHANNEL.send(new RiftParticlesPacket(Optional.of(current), Vec3.atCenterOf(block)), PacketDistributor.TRACKING_CHUNK.with(w.getChunkAt(block)));
                    w.playSound(null, block.getX() + 0.5, block.getY() + 0.5, block.getZ() + 0.5, SoundEvents.ENDERMAN_TELEPORT, SoundSource.BLOCKS, 1, 1);
                    Multiverse.CHANNEL.send(new RiftParticlesPacket(Optional.of(target), Vec3.atCenterOf(pos)), PacketDistributor.TRACKING_CHUNK.with(world.getChunkAt(pos)));
