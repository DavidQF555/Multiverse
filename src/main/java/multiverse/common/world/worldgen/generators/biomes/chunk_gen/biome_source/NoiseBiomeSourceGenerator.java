...
public class NoiseBiomeSourceGenerator implements BiomeSourceGenerator<MultiverseBiomeSource> {
    ...
    private static Climate.ParameterList<Holder<Biome>> parameters(Registry<Biome> registry, Registry<DimensionType> dimType, int minY, int maxY, long seed, double temperature, double humidity, MultiverseType type, HolderSet<Biome> biomes) {
    ...
    @Override
    public MultiverseBiomeSource generate(RegistryAccess access, long seed, RandomSource random, MultiverseType type, HolderSet<Biome> biomes) {
        GeneratorSettings settings = GeneratorSettings.getSettings();
        return new MultiverseBiomeSource(parameters(access.registryOrThrow(Registry.BIOME_REGISTRY), access.registryOrThrow(Registry.DIMENSION_TYPE_REGISTRY), minY, maxY, seed, settings.temperature(), settings.humidity(), type, biomes));
    }
