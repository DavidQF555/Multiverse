...
public class RiftExplosionParticlesPacket {
    ...
    private static final Function<FriendlyByteBuf, RiftExplosionParticlesPacket> DECODER = buffer -> new RiftExplosionParticlesPacket(buffer.readBoolean() ? Optional.of(buffer.readResourceKey(Registries.DIMENSION)) : Optional.empty(), new Vec3(buffer.readDouble(), buffer.readDouble(), buffer.readDouble()));
    ...
    private void handle(CustomPayloadEvent.Context context) {
        ClientHelper.addRiftExplosionParticles(from, center);
    }
