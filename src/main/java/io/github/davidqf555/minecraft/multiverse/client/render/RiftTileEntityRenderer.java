...
public class RiftTileEntityRenderer implements BlockEntityRenderer<RiftTileEntity> {
    ...
    protected double[][] calculateColors(int layers, double minA, double maxA, int center, int edge) {
        ...
        double[] target = new double[]{ARGB.red(center) / 255.0, ARGB.green(center) / 255.0, ARGB.blue(center) / 255.0};
        ...
        double[] rate = new double[]{
                (target[0] - ARGB.red(edge) / 255.0) / (layers - 1.0),
                (target[1] - ARGB.green(edge) / 255.0) / (layers - 1.0),
                (target[2] - ARGB.blue(edge) / 255.0) / (layers - 1.0)
        };
        double destA = 0;
        double factor = 1;
    ...
    private void drawQuad(VertexConsumer consumer, PoseStack stack, Vec3 p1, Vec3 p2, Vec3 p3, Vec3 p4, Vec3 offset, float alpha, float red, float green, float blue, boolean forward) {
        ...
        if (forward) {
            consumer.addVertex(pose, (float) p4.x(), (float) p4.y(), (float) p4.z()).setColor(red, green, blue, alpha);
            consumer.addVertex(pose, (float) p3.x(), (float) p3.y(), (float) p3.z()).setColor(red, green, blue, alpha);
            consumer.addVertex(pose, (float) p2.x(), (float) p2.y(), (float) p2.z()).setColor(red, green, blue, alpha);
            consumer.addVertex(pose, (float) p1.x(), (float) p1.y(), (float) p1.z()).setColor(red, green, blue, alpha);
