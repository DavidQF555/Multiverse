        ...
        if (target != current) {
            ...
            if (w != null) {
                ...
                if (isBreakable(w, s, block) && w.destroyBlock(block, false, entity)) {
                    Multiverse.CHANNEL.send(new RiftEffectPacket(Vec3.atCenterOf(block), SoundSource.BLOCKS, current), PacketDistributor.TRACKING_CHUNK.with(w.getChunkAt(block)));
                    Multiverse.CHANNEL.send(new RiftEffectPacket(Vec3.atCenterOf(pos), SoundSource.BLOCKS, target), PacketDistributor.TRACKING_CHUNK.with(world.getChunkAt(pos)));
