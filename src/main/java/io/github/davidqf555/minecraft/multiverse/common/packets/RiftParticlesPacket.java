...
public class RiftParticlesPacket {
    ...
    private static final Function<FriendlyByteBuf, RiftParticlesPacket> DECODER = buffer -> new RiftParticlesPacket(buffer.readBoolean() ? Optional.of(ResourceKey.create(Registry.DIMENSION_REGISTRY, buffer.readResourceLocation())) : Optional.empty(), new Vec3(buffer.readDouble(), buffer.readDouble(), buffer.readDouble()));
