...
public class RiftExplosionParticlesPacket {
    ...
    private static final Function<FriendlyByteBuf, RiftExplosionParticlesPacket> DECODER = buffer -> new RiftExplosionParticlesPacket(buffer.readBoolean() ? Optional.of(ResourceKey.create(Registry.DIMENSION_REGISTRY, buffer.readResourceLocation())) : Optional.empty(), new Vec3(buffer.readDouble(), buffer.readDouble(), buffer.readDouble()));
