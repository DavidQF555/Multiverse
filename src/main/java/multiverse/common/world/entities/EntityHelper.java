...
public final class EntityHelper {
    ...
    public static <T extends Entity> T randomSpawn(EntityType<T> type, ServerLevel world, BlockPos center, int min, int max, MobSpawnType spawn) {
        ...
        for (int i = 0; i < TRIES; i++) {
            ...
            if (SpawnPlacements.getPlacementType(type).isSpawnPositionOk(world, block, type) && canSpawnPosition(world, Vec3.atBottomCenterOf(block), type)) {
                T entity = type.create(world, null, block, spawn, false, false);
                if (entity != null) {
