...
public class WarpTeleporter implements ITeleporter {
    ...
    public static Entity warp(Entity entity, ResourceKey<Level> target) {
        ...
        if (copy != null) {
            Multiverse.CHANNEL.send(new RiftEffectPacket(pos, copy.getSoundSource(), target), PacketDistributor.TRACKING_CHUNK.with(from.getChunkAt(BlockPos.containing(pos))));
            Vec3 changed = copy.getEyePosition();
            Multiverse.CHANNEL.send(new RiftEffectPacket(changed, copy.getSoundSource(), current), PacketDistributor.TRACKING_ENTITY_AND_SELF.with(copy));
